...
def coarse_fine_volatility_corr(cf_corr,cf_corr_diff,file_name):
    plt.figure(dpi=150)
    plt.plot(list(range(-25,26,1)),cf_corr, '.',label=r'$\rho(k)$',)
    plt.plot(list(range(-25,26,1)),cf_corr_diff,'.',label=r'$\Delta\rho(k)$')
    plt.ylim(-0.2, 1.)
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    plt.xlim(-25,25,5)
    plt.xlabel('lag k',fontsize=20)
    plt.ylabel(r'$\rho(k)$',fontsize=40)
    plt.legend()
    plt.savefig(file_name+'.png',transparent=True)
    plt.close()

def Gain_or_loss_asymtric_distribution(gain_prob,loss_prob,file_name):
    plt.figure(dpi=150)
    plt.plot(gain_prob, '.',color='blue',label=r'$T_{wait}(+\theta)$')
    plt.plot(loss_prob,'.',color='red',label=r'$T_{wait}(-\theta)$')
    plt.xscale('log')
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    plt.xlabel(r'$time\ ticks\ \Delta t$',fontsize=20)
    plt.ylabel('return time probability',fontsize=20)
    plt.legend()
    plt.savefig(file_name+'.png',transparent=True)
    plt.close()